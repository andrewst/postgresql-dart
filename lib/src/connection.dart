...
class PostgreSQLConnection implements PostgreSQLExecutionContext {
  ...
  final StreamController<Notice> _notices = new StreamController<Notice>.broadcast();
  ...
  void _readData(List<int> bytes) {
    ...
    while (_framer.hasMessage) {
      ...
      try {
        ...
        } else if (msg is NoticeResponseMessage) {
          _notices.add(new Notice._(msg.fields));
...
class Notice {
  Notice._(List<NoticeOrErrorField> errorFields) {
